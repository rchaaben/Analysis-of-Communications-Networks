...
class CustomerArrival(SimEvent):
    ...
    def process(self):
        ...
        addToServer=self.sim.system_state.add_packet_to_server()
        if addToServer:
            self.sim.sim_state.packet_accepted()
            e=CustomerArrival(self.sim,self.timestamp+self.sim.sim_param.IAT)
            self.sim.event_chain.insert(e)  
            ST=random.randint(0,1000)
            eSC=ServiceCompletion(self.sim,self.timestamp+ST)
            self.sim.event_chain.insert(eSC)      
        else:
            addToQueue=self.sim.system_state.add_packet_to_queue()
            if addToQueue:
                self.sim.sim_state.packet_accepted()
                eCA=CustomerArrival(self.sim,self.timestamp+self.sim.sim_param.IAT)
                self.sim.event_chain.insert(eCA)
            else:
...
class ServiceCompletion(SimEvent):
    ...
    def process(self):
        ...
        if self.sim.system_state.buffer_content==0:
            ...
            eSC=ServiceCompletion(self.sim,self.timestamp+ST)
            self.sim.event_chain.insert(eSC)    
            #self.sim.server_busy=True
            #print self.sim.system_state.buffer_content, self.sim.system_state.server_busy
